...
def load_data(batch_size: int, val_split: float, test_split: float):
    ...
    indices = [i for i in train_set.indices if DATASET.annotations.iloc[i]['label'] == 0]
    n = int(len(indices) * REDUCTION_RATIO)
    remove = np.random.choice(indices, n, replace=False)

    train_reduced = [i for i in train_set.indices if i not in remove]
    train_set = Subset(DATASET, train_reduced)
