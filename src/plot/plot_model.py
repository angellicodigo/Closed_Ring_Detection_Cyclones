...
def plot(model_path: Path, window_size: float, radius: float) -> None:
    ...
    with torch.no_grad():
        ...
        for index, (datas, _) in tqdm(enumerate(DataLoader, start=0), total=len(DataLoader), desc="Plotting all predictions"):
            ...
            output = model(datas)
            fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(21, 9), subplot_kw={'projection': ccrs.PlateCarree()})
            row = df.iloc[index]
            file_path = os.path.join(PATH_DATASET, row['file_name'])
            with xr.open_dataset(file_path) as ds:
                plot_subplot(ax1, ds, row['lat'], row['lon'], radius, window_size)
                plot_subplot(ax2, ds, row['lat'], row['lon'], radius, window_size, output, device)
                plt.tight_layout()
...
def plot_subplot(ax: Axes, ds: xr.Dataset, center_lat: float, center_lon: float, radius: float, window_size: float,    output: Optional[List[Dict[str, torch.Tensor]]] = None, device: Optional[torch.device] = None):
    ...
    if output != None:
        ...
        canvas = np.ones((ds['lon'].shape[0], ds['lon'].shape[1], 3), dtype=np.uint8) * 255
        ...
        width = x2 - x1
        height = y2 - y1
        im = cv2.rectangle(canvas, (x1, y1), (x2, y2), color=(255, 0, 0), thickness=1) # type: ignore
        window_size += 10
    ...
    if output == None:
        ...
        quiver = ax.quiver(ds['lon'], ds['lat'], U, V, ds['wind_speed'], cmap='turbo', transform=ccrs.PlateCarree(), scale=500, pivot='mid')
        cbar = plt.colorbar(quiver)
        cbar.set_label("Wind Speed")

        min_lat, min_lon, max_lat, max_lon = get_boundary_box(ds, center_lat, center_lon, radius)
        width = max_lon - min_lon
        height = max_lat - min_lat
        bbox = patches.Rectangle( (min_lon, min_lat), width, height, linewidth=2, edgecolor='black', facecolor='none')
        ...
        pixel_top_left = np.full((ds['lat'].shape[0], ds['lat'].shape[1]), np.nan)
        pixel_top_left[row_top, col_top] = 1

        row_bot, col_bot = coords_to_pixels(
            ds, bot_right[1], bot_right[0], center_lat, center_lon)
        pixel_bot_right = np.full((ds['lat'].shape[0], ds['lat'].shape[1]), np.nan)
