...
def preprocess(radius: float, threshold: float, checkBBox: bool):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Preprocessing each file"):
        if row['cyclone_id'] not in OVER_LAND and (row['file_name'] not in EXCLUDE):
            if (row['lon'] != -np.inf) and (row['lat'] != -np.inf):
                ...
                with xr.open_dataset(file_path) as ds:
                    if np.count_nonzero(~np.isnan(ds['wind_speed'].values)) != 0:
                        if check_boundary(ds, row['lat'], row['lon']):
                            if check_within_swaths(row['file_name'], row['lat'], row['lon']):
                                input = {}
                                if checkBBox:
