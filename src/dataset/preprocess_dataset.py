...
def not_outside_swaths(ds: xr.Dataset, center_lat: float, center_lon: float) -> bool:
...
def within_swaths(filename: str, center_lat: float, center_lon: float) -> bool:
    for root, dirs, files in os.walk(PATH_EXTERNAL):
        for file in files: 
