...
import os
import xarray as xr
from config.utils import dist_bwt_two_points, get_boundary_box, calc_percent_valid, nearest_neighbors_indices
import numpy as np
import scipy.io
...
PATH_INFO = r'C:\Users\angel\VSCode\ML_for_Medicane_Wind_Rings\data\interim\annotations_all.txt'
...
def preprocess(radius: float, threshold: float, checkBBox: bool):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Creating boundary box for all files"):
        if row['cyclone_id'] not in OVER_LAND:
            if (row['lon'] != -np.inf) or (row['lat'] != -np.inf):
                file_path = os.path.join(PATH_DATASET, row['file_name'])
                with xr.open_dataset(file_path) as ds:
                    if check_boundary(ds, row['lat'], row['lon']) and check_within_swaths(row['file_name'], row['lat'], row['lon']):
                        input = {}
                        if checkBBox:
                            input = get_BBox_input(ds, row, radius, threshold)
                        else:
                            input = get_segmentation_input(ds, row)
                        if len(input) != 0:
...
def check_within_swaths(file_name: str, query_lat: float, query_lon: float) -> bool:
    for root, _, files in os.walk(PATH_EXTERNAL):
        for mat_name in files:
            if os.path.splitext(mat_name)[0] == file_name:
                mat_path = os.path.join(root, file_name)
                ...
                return not np.isnan(mat.wind_speed[index])
    return False


def get_BBox_input(ds: xr.Dataset, row: pd.Series, radius, threshold) -> dict:
    input = {}
...
def get_segmentation_input(ds: xr.Dataset, row: pd.Series) -> dict:
    input = {}
